---
layout: home
title: Forside
nav_order: 1
---

# 🌐 Velkommen til Alis Netværk Akademi

Find dokumentation, labs og guides inden for IT-infrastruktur

---

## 🚀 KATEGORIER

<div class="portal-card" onclick="window.location='/netværk/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">🌐</div>
    <div class="portal-content">
      <div class="portal-title">Netværk</div>
      <p>VLAN, OSPF, routing, switching og netværksdesign.</p>
    </div>
  </div>
</div>

<div class="portal-card" onclick="window.location='/firewall/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">🔥</div>
    <div class="portal-content">
      <div class="portal-title">Firewall</div>
      <p>pfSense, NAT, port forwarding, VPN og sikker trafikstyring.</p>
    </div>
  </div>
</div>

<div class="portal-card" onclick="window.location='/windows-server/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">🪟</div>
    <div class="portal-content">
      <div class="portal-title">Windows Server</div>
      <p>Active Directory, DNS, DHCP, filserver og domæneadministration.</p>
    </div>
  </div>
</div>

<div class="portal-card" onclick="window.location='/linux/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">🐧</div>
    <div class="portal-content">
      <div class="portal-title">Linux</div>
      <p>Linux-servere, SSH, terminal, services og administration.</p>
    </div>
  </div>
</div>

<div class="portal-card" onclick="window.location='/virtualisering/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">📦</div>
    <div class="portal-content">
      <div class="portal-title">Virtualisering</div>
      <p>Hyper-V, virtuelle maskiner, virtuelle netværk og labmiljøer.</p>
    </div>
  </div>
</div>

<div class="portal-card" onclick="window.location='/cloud/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">☁️</div>
    <div class="portal-content">
      <div class="portal-title">Cloud</div>
      <p>Azure, Microsoft 365, cloud-services og moderne IT-infrastruktur.</p>
    </div>
  </div>
</div>

<div class="portal-card" onclick="window.location='/sikkerhed/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">🔒</div>
    <div class="portal-content">
      <div class="portal-title">Sikkerhed</div>
      <p>Hardening, adgangskontrol, logging, overvågning og best practices.</p>
    </div>
  </div>
</div>

<div class="portal-card" onclick="window.location='/projekter/'" style="cursor:pointer;">
  <div class="portal-left">
    <div class="portal-icon">📁</div>
    <div class="portal-content">
      <div class="portal-title">Projekter</div>
      <p>Cases, dokumentation og komplette IT-infrastrukturprojekter.</p>
    </div>
  </div>
</div>

---

### 📌 Populære emner

- 🌐 VLAN-konfiguration
- 🌐 OSPF Routing
- 🔥 pfSense NAT
- 🔒 ACL-regler
- 🪟 Active Directory
- 🐧 SSH og Linux Services

---

> "Knowledge is power. Documentation is control."
